...
<div style="display: flex; justify-content: center; align-items: center; min-height: 100vh; padding: 1rem;">
    <div style="background: white; border-radius: 12px; padding: 2.5rem 2rem; box-shadow: 0 10px 40px rgba(0, 0, 0, 0.15); max-width: 400px; width: 100%;">
        ...
        <div style="text-align: center; margin-bottom: 2rem;">
            <h1 style="color: #667eea; font-size: 1.75rem; font-weight: 600; margin: 1rem 0 0.5rem;"><i class="fas fa-graduation-cap" style="font-size: 2.5rem; color: #667eea;"></i>{{ __('messages.welcome') }}</h1>
            <p style="color: #666; margin: 0.25rem 0; font-size: 0.95rem;">{{ __('messages.login_subtitle') }}</p>
        ...
        <form method="POST" action="{{ route('login') }}">
            ...
            <div style="margin-bottom: 1.25rem;">
                <label style="display: flex; align-items: center; gap: 0.5rem; font-size: 0.9rem; font-weight: 500; color: #333; margin-bottom: 0.5rem;">
                    <i class="fas fa-user" style="width: 16px; text-align: center;"></i> {{ __('messages.username') }}
                </label>
                <input type="text" name="username" value="{{ old('username') }}" required autofocus style="width: 100%; padding: 0.75rem 1rem; border: 1.5px solid #e0e0e0; border-radius: 6px; font-size: 0.95rem; box-sizing: border-box;">
                @error('username')
            ...
            <div style="margin-bottom: 1.25rem;">
                <label style="display: flex; align-items: center; gap: 0.5rem; font-size: 0.9rem; font-weight: 500; color: #333; margin-bottom: 0.5rem;">
                    <i class="fas fa-lock" style="width: 16px; text-align: center;"></i> {{ __('messages.password') }}
                </label>
        ...
        <div style="text-align: center; margin-top: 1.5rem;">
            <select onchange="window.location.href=this.value" style="padding: 0.5rem 1rem; border: 1px solid #e0e0e0; border-radius: 6px; font-size: 0.9rem; cursor: pointer; background: white; color: #667eea;">
                <option value="{{ route('language.switch', 'fr') }}" {{ app()->getLocale() == 'fr' ? 'selected' : '' }}>
                    🇫🇷 Français
                </option>
